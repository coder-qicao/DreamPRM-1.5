...
parser.add_argument("--save_every_iterations", type=int, default=3000)
...
parser.add_argument("--activation_function", type=str, default="Clip", help="LeakyReLU|ReLU|No|Clip|sigmoid")
...
class Upper(ImplicitProblem):
    ...
    # torch.cuda.empty_cache()
        if len(upper_loss) == len(meta_dataloader):
            ...
            wandb.log({"upper_loss": mean_outer_loss})
            upper_loss.clear()
        try:
            return {"loss": loss}
